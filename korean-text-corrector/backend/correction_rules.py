...
class CorrectionRules:
    ...
    SPACING_PATTERNS = [
        # 조사는 붙여쓰기
        (r'(\w+)\s+(은|는|이|가|을|를|에|에서|로|으로|와|과|도|만|까지|부터|조차|마저|밖에|뿐|요)', r''),
        
        # 보조용언은 띄어쓰기 (권장)
        (r'(\w+)(지다|하다|되다|싶다|있다|없다|같다|보다|만하다)', r' '),
        
        # 의존명사는 띄어쓰기
        (r'(\w+)(것|수|지|줄|만큼|뿐|대로|채|바|체)', r' '),
        
        # 단위명사는 띄어쓰기
        (r'(\d+)(개|명|마리|권|장|대|번|분|초|시간|일|년|월|원|달러)', r' '),
